...
import {ISP1VerifierGatewayEvents, ISP1VerifierGatewayErrors, VerifierRoute} from "../src/ISP1VerifierGateway.sol";
...
contract SP1VerifierV1 is ISP1VerifierWithHash {
    ...
    function VERIFIER_HASH() public pure returns (bytes32) {
        return
            0x19ff1d210e06a53ee50e5bad25fa509a6b00ed395695f7d9b82b68155d9e1065;
    }
...
contract SP1VerifierV2 is ISP1VerifierWithHash {
    ...
    function VERIFIER_HASH() public pure returns (bytes32) {
        return
            0xfd4b4d23a917e7d7d75deec81f86b55b1c86689a5e3a3c8ae054741af2a7fea8;
    }
...
contract SP1VerifierGatewayTest is
    Test,
    ...
    ISP1VerifierGatewayErrors
{
    ...
    function test_SetUp() public view {
        ...
        address verifier;
        bool frozen;
        (verifier, frozen) = SP1VerifierGateway(gateway).routes(
            bytes4(SP1VerifierV1(verifier1).VERIFIER_HASH())
        );
        assertEq(verifier, address(0));
        assertEq(frozen, false);

        (verifier, frozen) = SP1VerifierGateway(gateway).routes(
            bytes4(SP1VerifierV2(verifier2).VERIFIER_HASH())
        );
        assertEq(verifier, address(0));
        assertEq(frozen, false);
    }
    ...
    function test_AddRoute() public {
        ...
        bytes4 verifier1Selector = bytes4(
            SP1VerifierV1(verifier1).VERIFIER_HASH()
        );
        ...
        (address verifier, bool frozen) = SP1VerifierGateway(gateway).routes(
            verifier1Selector
        );
    ...
    function test_RevertAddRoute_WhenAlreadyExists() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(RouteAlreadyExists.selector, verifier1)
        );
        vm.prank(owner);
        SP1VerifierGateway(gateway).addRoute(verifier1);
    }
    ...
    function test_RevertFreezeRoute_WhenNoRoute() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(RouteNotFound.selector, verifier1Selector)
        );
        vm.prank(owner);
        SP1VerifierGateway(gateway).freezeRoute(verifier1Selector);
    }
    ...
    function test_RevertFreezeRoute_WhenRouteIsFrozen() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(RouteIsFrozen.selector, verifier1Selector)
        );
        vm.prank(owner);
        SP1VerifierGateway(gateway).freezeRoute(verifier1Selector);
    }
    ...
    function test_VerifyProof() public {
        ...
        SP1VerifierGateway(gateway).verifyProof(
            PROGRAM_VKEY,
            PUBLIC_VALUES,
            PROOF_1
        );
